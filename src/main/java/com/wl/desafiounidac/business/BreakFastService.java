...
public class BreakFastService {
	...
	public void createBreakFast(LocalDate date, @Valid Set<Integer> items, String collaboratorCpf, ItemService itemService,
		...
		// para cada item, adiciona na Tabela de Collaboradores e Itens os valores referentes
	...
	public void insertCollaboratorAndItems(@Valid BreakFastDto dto, CollaboratorService collabService, ItemService itemService)
			throws CollaboratorNotFoundException, ItemNotFindedException, CurrentBreakFastNotFoundException,
				CollaboratorRepetitionFindedException, ItemRepetitionFindedException {
		...
		for(Integer itemId : dto.getItemsId()){
			...
			repository.insertIntoCollabAndItems(SeqSql.SEQ_COLLAB_AND_ITEM.toString().toLowerCase(),
					itemId, breakFastId, dto.getCollaboratorCpf());
		}
		
	}

	public List<CollaboratorsAndItems> getCollabAndItemsOfBreakFast(Integer breakFastId, CollaboratorsAndItemsService collabsAndItemsRepository) {
	...
	private void verifyCollaboratorRepetition(String collaboratorCpf, Integer breakFastId) 
		...
		if(result > 0) {
			throw new CollaboratorRepetitionFindedException("Colaborador(a) já se encontra no café da manhã "
					+ "marcado para esta data");
		}
	}
	...
	private void verifyItemRepetition(Integer itemId, Integer breakFastId) throws ItemRepetitionFindedException {
		...
		if(result > 0) {
			throw new ItemRepetitionFindedException("Colaborador(a) já se encontra no café da manhã "
					+ "marcado para esta data");
		}
	}
}
